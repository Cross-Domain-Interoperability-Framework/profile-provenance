#!/usr/bin/env python3
"""
CDIF Provenance Profile JSON-LD Framing and Validation Script
...
ARRAY_PROPERTIES = [
    ...
    # NOTE: cdif:physicalDataType is intentionally NOT array-valued -- it is a
    # single value (string, @id object reference, or DefinedTerm) everywhere.
...
def main():
    parser = argparse.ArgumentParser(
        description='CDIF Provenance Profile JSON-LD Framing and Validation Tool',
...
Examples:
  ...
  python FrameAndValidate.py my-metadata.jsonld --frame cdifProvenance-frame.jsonld -o framed.json

  # Validate against Data Description Profile schema
  python FrameAndValidate.py my-metadata.jsonld --frame cdifProvenance-frame.jsonld -v --schema cdifProvenanceStructuredSchema.json

  # Full workflow with Data Description Profile files
  python FrameAndValidate.py my-metadata.jsonld --frame cdifProvenance-frame.jsonld -o framed.json -v --schema cdifProvenanceStructuredSchema.json
"""
    )
    ...
    parser.add_argument('--schema', default=str(SCRIPT_DIR / 'cdifProvenanceStructuredSchema.json'),
                        help='Path to JSON Schema (default: cdifProvenanceStructuredSchema.json)')
    parser.add_argument('--frame', default=str(SCRIPT_DIR / 'cdifProvenance-frame.jsonld'),
                        help='Path to JSON-LD frame (default: cdifProvenance-frame.jsonld)')
